<x-layouts.main>
    <main class="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-20 text-[#e0e1dd]">
        <!-- Header -->
        <div class="text-center mb-24" data-aos="fade-down">
            <h1 class="text-4xl sm:text-5xl md:text-7xl font-bold tracking-tighter mb-8">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-32">
            ...
            <div class="group p-6 sm:p-8 rounded-[2.5rem] bg-[#1b263b]/50 border border-[#415a77] hover:border-[#E05A47]/50 transition-all duration-500 hover:shadow-2xl hover:shadow-[#E05A47]/5 hover:-translate-y-2">
                <div class="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-3xl bg-[#0d1b2a] text-[#E05A47] flex items-center justify-center text-xl sm:text-2xl mb-6 sm:mb-8 group-hover:bg-[#E05A47] group-hover:text-white transition-all duration-500 shadow-inner">
                    <i class="fa-solid fa-{{ $service[0] }}"></i>
                </div>
                <h3 class="text-lg sm:text-xl font-bold mb-3">{{ $service[1] }}</h3>
                <p class="text-sm md:text-base text-[#778da9] leading-relaxed">{{ $service[2] }}</p>
            </div>
            @endforeach
        </div>
        ...
        <div class="mb-32">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                ...
                <div class="p-6 sm:p-8 md:p-10 rounded-[2.5rem] bg-[#1b263b] border {{ $loop->iteration == 2 ? 'border-[#E05A47] shadow-2xl shadow-[#E05A47]/10' : 'border-[#415a77]' }} relative transition-transform hover:scale-[1.02]">
                    @if($loop->iteration == 2)
                    ...
                    @endif
                    <h3 class="text-lg sm:text-xl font-bold mb-2 opacity-80">{{ $pkg[0] }}</h3>
                    <p class="text-3xl sm:text-4xl md:text-5xl font-bold text-[#E05A47] mb-8">{{ $pkg[1] }}</p>
        ...
        <div class="p-6 md:p-12 rounded-[3rem] bg-[#1b263b]/30 border border-[#415a77] backdrop-blur-sm">
            <h2 class="text-2xl sm:text-3xl font-bold mb-12">Expertise Across The Board</h2>
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach(['SEO Optimization', 'Website Maintenance', 'Performance Testing', 'Technical Support', 'Migration Services', 'Strategic Consulting'] as $item)
                    <div class="flex items-center gap-4 group">
                        <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#0d1b2a] flex items-center justify-center border border-[#415a77] group-hover:border-[#E05A47] transition-colors">
                            <i class="fa-solid fa-plus text-[10px] sm:text-xs text-[#E05A47]"></i>
                        </div>
